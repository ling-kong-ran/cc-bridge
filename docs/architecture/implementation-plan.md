...
- 收敛 `static/app.js` 中导航 wrapper，移除剩余本地页面切换、导航事件绑定和移动端侧栏 fallback 实现，统一委托 `static/js/navigation.js`。
...
- 收敛 `static/app.js` 中完成后历史同步 wrapper，移除剩余本地延迟同步 timer fallback 实现，统一委托 `static/js/completion-sync.js`。
