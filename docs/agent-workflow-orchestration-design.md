...
第一版交互以“可运行、可观察、可审批”为主，不做复杂拖拽编辑。
...
1. 点击 `从模板创建`。
2. 弹出模板选择：代码修改、PR Review、发布前检查、记忆整理。
3. 选择模板后生成一份 workflow draft。
4. 用户可在 Inspector 修改节点标题、prompt、cwd、model、输出变量。
5. 点击 `保存草稿` 调用 `POST /api/workflows` 或 `PUT /api/workflows/{id}`。
6. 保存成功后更新左侧列表。
