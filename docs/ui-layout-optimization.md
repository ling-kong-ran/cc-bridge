...
问题：完整会话列表继续放在左侧 sidebar 时，会和运行设置、状态信息挤在同一列；列表宽度有限，时间、模型、费用、token 和操作按钮难以形成清晰层级，也会让 Chat 页面同时承担“会话管理”和“对话”两种焦点。

已调整：

- 左侧导航 rail 新增 `Sessions` 入口。
- 完整会话列表从左侧 sidebar 移到独立 Sessions 页面。
- Sessions 页面提供标题说明、搜索、新建会话、会话数量和按时间倒序的会话列表。
- 每个会话项右侧集中放置 `PIN / DIR / EDIT / ×` 操作，点击主体仍进入 Chat 并恢复会话。
- Chat 页面不再展示完整历史列表，顶部也不再额外显示四个会话管理按钮，继续专注当前消息流、输入区和 Workspace。
- Home 欢迎页的 Recent 保留轻量入口，用于快速恢复最近会话。
- 左侧栏继续向纯导航 rail 收敛，移除左侧“重新开始”入口。
- Runtime Settings 从左侧栏迁入 Chat 输入区上方，保留为当前会话可随时调整的折叠面板。

目标效果：会话管理与对话页面分离，Sessions 页面负责浏览、搜索和维护历史会话；Chat 页面只负责当前会话内容，整体信息架构更清楚。
...
Artifacts 和 Scheduled 的布局统一已完成，两者继续保留各自业务结构，但共享页面 shell、header、toolbar/card 的视觉规则。

已调整：

- Artifacts / Scheduled 页面使用一致的最大宽度、左右 padding 和纵向间距。
- 两个页面 header 的高度、kicker / title / description 间距统一。
- Artifacts toolbar 改为与 Settings tabs 类似的弱卡片工具条，filter tab 使用统一 pill active 状态。
- Artifacts empty state、图片卡片、文件/链接行补齐统一边框、圆角和间距。
- Scheduled list/form card 圆角和边框弱化为与 Artifacts card 一致。
- Scheduled form actions 统一右对齐并支持换行，移动端布局继续降为单列。

目标效果：Artifacts 和 Scheduled 看起来像同一套工具页，而不是两个独立拼接出来的页面。
